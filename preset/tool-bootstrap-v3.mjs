...
export function apply(ctx, config) {
  ...
  // Deployment plugin tools that stay directly visible after promotion
  // (local addition): the anchored resident set is intentionally small, but
  // host-installed plugin tools the user relies on must not be hidden behind
  // dev_tool_search — otherwise the model cannot discover them on its own.
